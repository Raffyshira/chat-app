...
export const get = query({
   ...
   handler: async (ctx, args) => {
      ...
      // Ambil semua percakapan berdasarkan membership
      ...
      const conversationsWithDetails = await Promise.all(
         conversations.map(async (conversation, index) => {
            ...
            // Untuk percakapan yang bukan grup, ambil anggota lain
            if (!conversation.isGroup) {
               const otherMembership = allConversationMemberships.find(
                  membership => membership.memberId !== currentUser._id
               );

               // Jika anggota lain tidak ditemukan, kembalikan null
               if (!otherMembership) {
            ...
            // Jika grup, kembalikan percakapan dengan semua anggota
            return {
               conversation,
               lastMessage,
               unseenCount: unseenMessages.length,
