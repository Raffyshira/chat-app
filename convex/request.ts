...
export const accept = mutation({
   args: {
      id: v.id("requests") // Gunakan `v.id("requests")` untuk tipe yang benar
   },
   handler: async (ctx, args) => {
      const identity = await ctx.auth.getUserIdentity();
      ...
      await ctx.db.insert("friends", {
         ...
         conversationsId: conversationId // Sesuaikan nama kolom di `friends`
      });
      ...
      await ctx.db.delete(args.id); // Gunakan args.id untuk menghapus request
   }
});
